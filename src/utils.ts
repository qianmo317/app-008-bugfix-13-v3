...
export function nextBoxSeq(task: MoveTask, roomTo: string): number {
  const sameRoom = task.boxes.filter((b) => b.roomTo === roomTo);
  const numbered = sameRoom.map((b) => parseInt(b.code.split('-').pop() || '0', 10));
  let max = 0;
...
export function generateBoxCode(task: MoveTask, roomTo: string): string {
  ...
  const seq = nextBoxSeq(task, roomTo);
